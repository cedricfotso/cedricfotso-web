...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
	...
	const projectRoutes = projects.map((slug) => ({ url: `${base}/etudes-de-cas/${slug}`, lastModified: new Date() }))
	const postRoutes = posts.map((slug) => ({ url: `${base}/ecrits/${slug}`, lastModified: new Date() }))
