					...
					{Object.values(METIERS).map((m) => (
						<Card key={m.slug} className="flex h-full flex-col">
							...
							<ul className="mt-5 space-y-1.5 text-[14px]">
								{m.livrables.map((l) => (<li key={l.titre} className="flex gap-2"><span className="text-muted">·</span><span>{l.titre}</span></li>))}
							</ul>
							<div className="mt-auto pt-6"><AccentLink href={`/metiers/${m.slug}`}>En savoir plus</AccentLink></div>
						</Card>
					))}
				</div>
			</Section>
